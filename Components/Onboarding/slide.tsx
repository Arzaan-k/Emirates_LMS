...
export default function Slide({
  ...
  const handlePress = () => {
    ...
    if (index === totalSlides - 1) {
      ...
      // or if you want to clear onboarding from history:
      // navigation.reset({
      //   index: 0,
      //   routes: [{ name: "Login" }],
      // });
  ...
  const slideColor: string =
    typeof slide.color === "string" ? slide.color : String(slide.color ?? "#000");

  return (
    <View style={[StyleSheet.absoluteFillObject, { backgroundColor: slideColor }]}>
      <Svg style={StyleSheet.absoluteFill}>
        <Defs>
          <RadialGradient id="gradient" cx="50%" cy="35%">
            <Stop offset="0%" stopColor={slideColor} />
            <Stop offset="100%" stopColor={slideColor} />
          </RadialGradient>
        </Defs>
      ...
      {index <= totalSlides - 1 && (
        <LinearGradient colors={["#6D55FE", "#8976FC"]} style={styles.nextButton}>
          <Pressable style={styles.nextButtonInner} onPress={handlePress}>
            <Text style={styles.nextButtonText}>
              {index === totalSlides - 1 ? "Start" : "Next"}
            </Text>
          </Pressable>
        </LinearGradient>
      )}
